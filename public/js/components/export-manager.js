...
async function generateExcelFile(exportData, filename, userType) {
    ...
    for (let sheetIndex = 0; sheetIndex < sheetNames.length; sheetIndex++) {
        ...
        // 工作表“角色”：样式按 kind 应用，不依赖工作表位置序号
        // detail=每日排课明细, summary=授课/上课汇总, stats=授课/上课统计, raw=排课原始记录
        const SHEET_KINDS = {
            '每日排课明细': 'detail',
            '教师授课汇总': 'summary',
            '学生上课汇总': 'summary',
        ...
        worksheet.columns = headers.map((header, colIndex) => {
            ...
            // detail（每日排课明细）：特定列宽
            if (kind === 'detail' && firstSheetColumnWidths[header]) {
                width = firstSheetColumnWidths[header];
            }
            // summary（授课/上课汇总）：汇总30、核对20、备注60，其余列15
            ...
            // raw（排课原始记录）：上课地点40，更新时间/课程状态自动更新时间20
            else if (kind === 'raw') {
                if (header === '上课地点') {
                    width = 40;
        ...
        cleanData.forEach((rowData, rowIndex) => {
            ...
            excelRow.eachCell({ includeEmpty: true }, (cell, colNumber) => {
                ...
                // detail（每日排课明细）费用/周汇总列：纯数字以数字形式存储
                if (kind === 'detail' && (header === '费用' || header === '周汇总')) {
                ...
                // raw（排课原始记录）：已取消状态整行斜体灰色
                if (kind === 'raw' && String(originalRow['状态'] || '').includes('已取消')) {
                ...
                // 费用/汇总/统计列特殊对齐（按 kind + 列名判断，避免依赖列位置）
                const s2RightBottom = ['试教', '入户', '评审', '集体活动', '咨询', '汇总'];
                const isNameHeader = header === '教师姓名' || header === '学生姓名' || header === '姓名';
                const isFinanceHeader = header === '费用' || header === '周汇总' || header === '汇总';
                const needsRightBottom = isFinanceHeader
                    || (kind === 'summary' && s2RightBottom.includes(header))
                    || (kind === 'stats' && !isNameHeader && header !== '备注' && header !== '核对');
                ...
                // 长文本靠左对齐（原始记录表除外）
                if (kind !== 'raw' && strValue.length > 10 && !needsRightBottom) {
                    cell.alignment = { ...cell.alignment, horizontal: 'left' };
                }

                // summary（授课/上课汇总）：核对列水平垂直居中
                if (kind === 'summary' && header === '核对') {
                    cell.alignment = { ...cell.alignment, horizontal: 'center', vertical: 'middle' };
                }

                // summary（授课/上课汇总）：汇总列靠右显示，文本中的数字加粗（rich text）
                if (kind === 'summary' && header === '汇总') {
                ...
                // 纯数字加粗：summary/stats 表的数值列（姓名/备注/核对 列除外）
                const isPureNumberCell = strValue.trim() !== '' && /^\d+(\.\d+)?$/.test(strValue.trim());
                if (isPureNumberCell && (kind === 'summary' || kind === 'stats') && !isNameHeader && header !== '备注' && header !== '核对') {
                    cell.font = { ...cell.font, bold: true };
                }
            });
        });
