...
function aggregateTeacherStats(rawData, studentName = '全部学生', state = {}) {
    ...
    statsMap.forEach(stat => {
        ...
        if (finalVisit > 0) remarksDetails.push(`${finalVisit}次入户`);
        if (finalReview > 0) remarksDetails.push(`${finalReview}次评审`);
        if (finalConsult > 0) remarksDetails.push(`${finalConsult}次咨询`);
        if (finalGroup > 0) remarksDetails.push(`${finalGroup}次集体活动`);
        const remarksDetailsStr = remarksDetails.length > 0 ? remarksDetails.join('、') : '无';

        const surname = stat.name ? stat.name.charAt(0) : '';
        const remarks = `${surname}老师好！${cleanDateRange}期间，您在[${studentName}]处入户等相关数据为 ：${remarksDetailsStr}。请问是否正确？`;
