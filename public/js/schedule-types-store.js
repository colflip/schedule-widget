...
(function() {
    ...
    const ScheduleTypesStore = {
        ...
        async ensureLoaded() {
            ...
            if (restored && this.list.length > 0) {
                console.log('[ScheduleTypesStore] 从缓存加载了', this.list.length, '个排课类型');
                return this.list;
            }
            ...
            try {
                if (!window.apiUtils || typeof window.apiUtils.get !== 'function') {
                    console.warn('[ScheduleTypesStore] apiUtils 不可用');
                    return [];
                }
                ...
                if (Array.isArray(fetched)) {
                    this.load(fetched);
                    console.log('[ScheduleTypesStore] 从API加载了', fetched.length, '个排课类型');
                    return this.list;
                }
            } catch (error) {
                console.warn('[ScheduleTypesStore] 从API获取排课类型失败:', error);
            }
    ...
    console.log('[ScheduleTypesStore] 已初始化');
})();
