...
// 导入时间槽工具函数
import { getTimeSlotFromStartTime } from './utils/timeSlots.js';
...
function buildScheduleCard(schedule) {
    ...
    // Resolve Chinese type name
    let typeLabel = '课程';
    let typeId = schedule.course_id || schedule.schedule_type_id || schedule.type_id;
    if (window.ScheduleTypesStore && typeId) {
        const typeInfo = window.ScheduleTypesStore.getById(typeId);
        if (typeInfo) typeLabel = typeInfo.description || typeInfo.name;
    }
    if (typeLabel === '课程') {
        typeLabel = schedule.schedule_type_cn || schedule.schedule_types || schedule.schedule_type || '课程';
    }
