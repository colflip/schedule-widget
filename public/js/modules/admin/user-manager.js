...
// Retry helper (local version or updated to use utils if available)
...
export function setupUserEventListeners() {
    ...
    userTabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            // Updating header title is handled by the caller or UI helper, but we could trigger an event
            const headerTitle = document.querySelector('.dashboard-header h2');
            if (headerTitle) headerTitle.textContent = '用户管理';
        });
    });
...
function toggleContactFields(userType) {
    ...
    if (userType === 'admin') {
        ...
        if (userType === 'teacher' && groups.work) groups.work.style.display = 'block';
        if (userType === 'student' && groups.visit) groups.visit.style.display = 'block';
    }
}
