(function () {
  ...
  // 可访问配色（WCAG AA 友好倾向）：以蓝/橙为主，辅以互补色
  ...
  // 统一图表字体：读取 CSS 变量
  try {
    const chartFont = getCssVar('--chart-font-family', null);
    if (chartFont && window.Chart && Chart.defaults && Chart.defaults.font) {
      Chart.defaults.font.family = chartFont;
    }
  } catch (_) { }

  function getPalette(opts) {
    const theme = (opts && (opts.palette || opts.theme)) || 'accessible';
    switch (theme) {
      case 'accessible':
      default: return ACCESSIBLE_PALETTE;
    }
  }
  ...
  function renderStackedBarChart(canvasId, labels, stacks, opts = {}) {
    try {
      ...
      const el = document.getElementById(canvasId);
      if (!el) {
        console.warn('renderStackedBarChart: Canvas element not found:', canvasId);
        return;
      }
      ...
      const datasets = normalizedStacks.map((s, i) => ({
        ...
        borderColor: colorFor(s.label, i),
        borderWidth: 1,
        barPercentage: 0.85, // 微调以获得更好的视觉效果
