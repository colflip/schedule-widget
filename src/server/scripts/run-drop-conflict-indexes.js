...
const migrationFile = path.join(__dirname, '../db/migrations/20251108_drop_conflict_indexes.sql');
...
async function runDropConflictIndexes() {
  try {
    console.log('读取迁移脚本:', migrationFile);
    const sql = fs.readFileSync(migrationFile, 'utf-8');
    const statements = splitSqlStatements(sql);
    console.log('开始执行删除冲突性索引迁移，共', statements.length, '条语句');
    ...
    console.log('删除冲突性索引迁移执行完成');
  } catch (err) {
    console.error('删除冲突性索引迁移执行失败:', err);
    process.exitCode = 1;
  } finally {
...
if (require.main === module) {
  runDropConflictIndexes();
}

module.exports = runDropConflictIndexes;
